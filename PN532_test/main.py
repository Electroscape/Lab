...
def compare_test_result(line):
    # test shall not be compared yet
    if datetime.now().timestamp() - test_timestamp < 1:
        return

    res_keyword = "!Result: "
    if re.match(res_keyword, line) is not None:
        res_number = int(re.sub(res_keyword))
        for i, status in enumerate(expected_result):
            if status:
                res_number -= pow(2, i)
        if res_number == 0:
