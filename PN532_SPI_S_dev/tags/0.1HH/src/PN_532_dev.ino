...
String versionDate = "09.03.2020";
String version = "version 0.1HH";
...
CRGB LED_STRIPE_3[2*NR_OF_LEDS];
...
Adafruit_PN532 RFID_READERS[RFID_AMOUNT] = {RFID_0, RFID_1, RFID_2, RFID_3}; //
static char RFID_solutions[RFID_AMOUNT][RFID_SOLUTION_SIZE] = {"AH", "SD", "GF", "PA"}; //
char RFID_reads[RFID_AMOUNT][RFID_SOLUTION_SIZE] = {};//

uint8_t keya[6] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
#define RFID_DATABLOCK      1
int cards_solution[RFID_AMOUNT]; //0 no card, 1 there is card, 2 correct card
bool runOnce = false;
...
void setup() {
    ...
    Serial.print("Setting up so many led PWMs: "); Serial.println(LED_PWMS);
    for (int led_pwm_nr=0; led_pwm_nr<LED_PWMS; led_pwm_nr++) {
        NeoPixel_Init(led_pwm_nr);
    };
...
void set_buffers() {
    // you need to call memset inside a function so here it is
    memset(cards_solution, 0, RFID_SOLUTION_SIZE);
    memset(RFID_reads, "XX", RFID_SOLUTION_SIZE);
}
    ...
    for (int reader_nr=0; reader_nr<RFID_AMOUNT; reader_nr++) {
        ...
        if (success) {
            ...
            if (!read_PN532(reader_nr, data, uid, uidLength)) {
                ...
                //Update data
                data[RFID_SOLUTION_SIZE] ='\0';
                strcpy(RFID_reads[reader_nr], (char*)data);
            }
...
bool RFID_Status(){
    ...
    for (uint8_t i=0; i < RFID_AMOUNT; i++){
        ...
        Serial.print("reader ");Serial.print(i);Serial.print(": ");
        if (cards_solution[i]==2)
            Serial.println("Correct Card");
        else if (cards_solution[i]==1){
            Serial.print(RFID_solutions[i]);Serial.print("!=");Serial.println(RFID_reads[i]);
    ...
    if (sum==8)
    {
      Serial.println(" == Correct Solution\n");
      return true;}
    else
    {
      Serial.println("\n");
...
void Update_serial(){
  ...
  if (delayRunning && ((millis() - delayStart) >= 5000)) {
    delayStart = millis();
    Serial.print("Refresh ");Serial.println(refresh);
    refresh++;
    RFID_Status();
  }
}
...
bool data_correct(int current_reader, uint8_t *data) {
    ...
    if (result == current_reader) {
        ...
        cards_solution[current_reader] = 2;
        return true;
    } else {
        //Serial.println(" Wrong");
        cards_solution[current_reader] = 1;
        return false;
    }
}
